...
PROPDMGEXP includes `r lunghezza_prop` values, namely:  
`r unique_prop2_measure`. You will have noticed that one value is left blank in this list. In $ amounts, these values correspond, respectively, to the following vector of values:  
...
In turn, CROPDMGEXP includes `r lunghezza_crop` values, namely: `r unique_crop2_measure`. Here too we find a blank value. In $ amounts, these values correspond, respectively, to the following vector of values: [0,1000,1000000,1000,0].  
...
There are `r b` NA values in our final data table, so missing values will be of no concern.
...
b<-sum(a)
```
There are `r b` NA values in our table for 2010, so missing values will be of no concern in our California Intermission.
...
From running the above code, we learn that across the United States, the weather events classified as `r max_disaster_fatalities[1,1]` were responsible for the highest number of fatalities from 1989 to 2011, for a total of `r max_disaster_fatalities[1,2]` victims. The events classified as `r max_disaster_injuries[1,1]` were responsible for the highest number of injuries, for a total of `r max_disaster_injuries[1,3]` victims.  
...
In the light of the following figure, containing two panels, it is easy to confirm the above remark that the events classified as `r finalDataFat[1,1]` were by far the leading cause of fatalities in the time period under examination, followed by the events classified as `r finalDataFat[2,1]` and as `r finalDataFat[3,1]`.
As to injuries, the events classified as `r finalDataInj[1,1]` hold the first place by a small margin, followed, at a virtually equal order of magnitude, by those classified as `r finalDataInj[2,1]`.
...
In the light of the following figure, containing two panels, it is easy to see that the events classified as `r finalDataPropDmgs[1,1]` were by far the worst cause of damages to properties, followed, on a proportionally microscopic order of magnitude, by the events classified as `r finalDataPropDmgs[2,1]` and as `r finalDataPropDmgs[3,1]`.  
As to damages to crops, the events classified as `r finalDataCropDmgs[1,1]` were the leading cause, followed by the events classified as `r finalDataCropDmgs[2,1]` and as `r finalDataCropDmgs[3,1]`.
But the difference in the order of magnitude between the largest damages to properties and to crops is impressive: the largest damages to properties total a figure which is `r ratio` greater than the largest damages to crops. Here are the graphs that tell this story:
...
As seen in the following figure, containing two plots, the highest number of injuries in California was caused by the events classified as `r ten_top_inj[1,1]` in 2010, followed by the events classified as `r ten_top_inj[2,1]` and as `r ten_top_inj[3,1]`.  
In turn, the worst damages to properties were caused by the evenys classified as `r ten_top_propDmgs[1,1]`, followed by those classified as `r ten_top_propDmgs[2,1]` and as `r ten_top_propDmgs[3,1]`.    
...
A final caveat: It would seem that the 2010 California data for damages to crops are scarcely reliable, as the entire data set for California contains only `r availableCropDmgs` datum. According to this datum, as we said, flash floods caused $ `r massimocropDmgs` in damages to crops. Yet, the fact that regarding damages to crops in 2010 in California, data are not available for any other weather event suggests that we are dealing here with an unreliable source. 